...
const IPFS_API_URL = import.meta.env.VITE_IPFS_API_URL || 'https://api.pinata.cloud';
...
export const usePropertyImages = () => {
  ...
  const uploadPropertyImages = useCallback(async (files: File[]): Promise<string[]> => {
    const hashes: string[] = [];
  ...
  const createPropertyMetadata = useCallback(async (propertyData: {
    title: string;
    description: string;
    images: string[];
    location: string;
...
export const useDocumentStorage = () => {
  ...
  const uploadContractData = useCallback(async (contractData: {
    rentalId: number;
  ...
  const uploadDisputeEvidence = useCallback(async (evidenceData: {
    rentalId: number;
    initiator: string;
    evidence: {
      description: string;
